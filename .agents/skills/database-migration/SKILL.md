...
├── 00_base_schema.sql       ← Core tables (households, app_users, expenses)
├── 01_locations.sql         ← B2B location primitives
├── 02_expenses_update.sql   ← Expense table B2B columns (location_id, currency)
└── 03_code_db_handshake.sql ← get_household_bundle RPC
```
...
- [ ] Does every policy use `get_my_household()` for isolation?
- [ ] Are you adding a column to `expenses`? Check `save_receipt_v3` signature still matches.
- [ ] Does this break the `get_household_bundle` RPC return shape?
...
CREATE POLICY "Household isolation" ON public.your_table
...
| RPC | Purpose | Status |
|-----|---------|--------|
| `get_household_bundle` | Platinum Handshake — frontend init | CANONICAL |
| `save_receipt_v3` | All financial write mutations | CANONICAL |
| `get_my_household()` | Helper — returns `household_id` from JWT | CANONICAL |
| `save_receipt_v2` | Deprecated — household era, no location/currency | DEPRECATED |
...
- **Never** hardcode `household_id` values in migrations — use `get_my_household()`.

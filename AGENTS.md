...
| **SOLID** | 🟢 **Hardened** | Domain logic isolated in `modules/`. Business logic decoupled from UI via headless hooks. SRP extractions: scanner-client split into 3 (V-88), insight-queries types extracted (V-89), dispatchDecision split into 2 (V-90), webhook/route split into 4 utilities (V-54). OCP: decision-router accepts new handlers via registry (V-87). LSP compliance: workflow action services return standard failure structures instead of throwing raw exceptions (V-58). DIP: services injected via constructor pattern (V-85). Factory pattern for Supabase clients enforced across all server actions. Phase 4: computeFCVReport split into computeAggregates/computePerIngredient/computeTemporalAnalysis (V-8) and monolithic `finance.ts` god object deconstructed into cohesive sub-modules (V-52). |
